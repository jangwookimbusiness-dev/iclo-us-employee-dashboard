...
MEMBER_CASES = [
    ...
    ("booking", "booking[].label",
     r"^(\s*label: )Find a dentist$", r"\g<1>변조된 넘김 라벨", None,
     "Find a dentist", "변조된 넘김 라벨"),
    ("booking", "booking[].knows",
     r"^(\s*knows: )앱이 넘겼다는 것까지\. 전화 저편은 보이지 않는다$",
     r"\g<1>변조된 한계 진술", None,
     "앱이 넘겼다는 것까지.", "변조된 한계 진술"),
    ("booking_caveat", "booking_caveat",
     r"^(\s*caveat: \|\n\s*)기록되는 것은 당신이 말한 것입니다\.",
     r"\g<1>변조된 예약 단서.", None,
     "기록되는 것은 당신이 말한 것입니다.", "변조된 예약 단서"),
