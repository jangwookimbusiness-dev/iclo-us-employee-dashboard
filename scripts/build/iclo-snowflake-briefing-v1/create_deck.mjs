...
const TMP = path.join(ROOT, "tmp/iclo-snowflake-briefing-v1");
const FINAL = path.join(ROOT, "output/pptx/ICLO-Snowflake-External-Briefing-Addendum-v1.pptx");
const IMG_OVERVIEW = path.join(TMP, "assets/employer-dashboard-overview.png");
const IMG_SUPPRESSED = path.join(TMP, "assets/employer-dashboard-suppressed.png");
...
function addHeader(slide, title, takeaway, labels, slideNumber, appendix = false) {
  ...
  addText(slide, appendix ? "APPENDIX" : "EXTERNAL BRIEFING ADDENDUM", {
  ...
  addText(slide, "ICLO x Snowflake | External briefing addendum", {
    left: 64, top: 687, width: 420, height: 18,
...
function connect(slide, from, to, options = {}) {
  return slide.shapes.connect(from, to, {
    ...
    head: options.noHead ? { type: "none" } : { type: "arrow", width: "med", length: "med" },
  });
}
...
// Slide 1
{
  ...
  addText(slide, "EXTERNAL BRIEFING ADDENDUM", { left: 64, top: 52, width: 360, height: 24 }, {
    fontSize: 12, bold: true, color: C.sky,
  });
  addText(slide, "ICLO x Snowflake", { left: 64, top: 112, width: 600, height: 64 }, {
    fontSize: 50, bold: true, color: C.white,
  });
  addText(slide, "Building the Evidence Layer for\nEmployer Dental Benefits", {
    left: 64, top: 178, width: 920, height: 125,
  }, { fontSize: 45, bold: true, color: C.white });
  addText(slide, "ICLO turns employee dental-benefit navigation into claims-confirmed, privacy-governed evidence.", {
    left: 64, top: 318, width: 1060, height: 58,
  }, { fontSize: 22, color: "#D9E2F0" });

  const boxes = [
    { x: 64, w: 298, title: "Employee experience", body: "Benefit understanding\nEmployee-initiated support", fill: C.paleCoral, line: C.coral },
    { x: 432, w: 398, title: "Snowflake governed evidence layer", body: "Eligibility + plan context + events + claims", fill: C.paleSky, line: C.sky },
    { x: 900, w: 316, title: "Employer aggregate outcome", body: "Claims-confirmed evidence\nNo individual PHI", fill: C.paleTeal, line: C.teal },
  ];
  ...
  const legend = [
    ["CONFIRMED", C.navy2], ["DESIGN PRINCIPLE", C.teal], ["HYPOTHESIS", C.amber], ["REQUEST", C.sky],
  ];
...
// Slide 3
{
  ...
  addText(slide, "Provisional ICLO redline pending U.S. regulatory counsel: keep signal, navigation and referral events separate; record employee action and consent timestamps.", {
    left: 270, top: 522, width: 900, height: 34,
...
// Slide 4
{
  ...
  addBox(slide, { left: 1092, top: 230, width: 124, height: 102 }, { fill: C.paleTeal, line: C.teal, lineWidth: 1.5 });
  addText(slide, "LOW\nTURNOVER", { left: 1104, top: 251, width: 100, height: 48 }, { fontSize: 14.5, bold: true, color: C.teal, alignment: "center" });
  addBox(slide, { left: 1092, top: 356, width: 124, height: 102 }, { fill: C.paleAmber, line: C.amber, lineWidth: 1.5 });
  addText(slide, "HIGH\nTURNOVER", { left: 1104, top: 377, width: 100, height: 48 }, { fontSize: 14.5, bold: true, color: C.amber, alignment: "center" });
...
// Slide 6
{
  ...
  const suppressed = await readImageBytes(IMG_SUPPRESSED);
  slide.images.add({
    blob: overview, contentType: "image/png", alt: "ICLO employer dashboard overview using synthetic data",
    fit: "contain", position: { left: 64, top: 205, width: 900, height: 288 }, geometry: "roundRect", borderRadius: "rounded-lg",
  });
  addBox(slide, { left: 64, top: 205, width: 900, height: 288 }, { fill: "none", line: C.line, lineWidth: 1.5 });

  const annotations = [
    ["Synthetic label", C.coral],
    ["Employee / member lenses", C.navy],
    ["Eligibility + claims dates and lag", C.sky],
    ["Completeness indicator", C.teal],
    ["Aggregate only + n >= 20 + no PHI", C.amber],
  ];
  annotations.forEach(([label, color], i) => {
    addDot(slide, 994, 211 + i * 55, i + 1, color);
    addText(slide, label, { left: 1032, top: 213 + i * 55, width: 184, height: 42 }, { fontSize: 14.5, bold: true, color: C.ink });
  });

  addText(slide, "Live click demo", { left: 64, top: 518, width: 180, height: 22 }, { fontSize: 14, bold: true, color: C.teal });
  addText(slide, dashboardUrl, { left: 64, top: 547, width: 610, height: 30 }, { fontSize: 15, color: C.sky });
  addText(slide, "The inset shows the policy firing for a 14-person group.", { left: 64, top: 588, width: 500, height: 34 }, { fontSize: 16, color: C.muted });
  slide.images.add({
    blob: suppressed, contentType: "image/png", alt: "Dashboard suppression state for a group below 20",
    fit: "contain", position: { left: 650, top: 512, width: 566, height: 181 }, geometry: "roundRect", borderRadius: "rounded-lg",
  });
  addBox(slide, { left: 650, top: 512, width: 566, height: 181 }, { fill: "none", line: C.amber, lineWidth: 1.5 });
  addText(slide, "Suppression-state proof", { left: 666, top: 518, width: 240, height: 22 }, { fontSize: 13, bold: true, color: C.amber });

  const note = "화면의 수치는 ICLO 성과가 아니라 buyer가 metric 정의와 data quality를 검증하는 방식의 데모다. 직원 기준 참여 분모와 covered member-month 기준 비용 분모를 분리하고, 기준일, claims lag, completeness, suppression을 동시에 노출한다. employer는 개인 PHI나 개인별 routing 정보를 보지 않는다.";
...
// Slide 7
{
  ...
  addText(slide, "Blue Cross Blue Shield: 33 independent, locally operated companies", { left: 648, top: 216, width: 548, height: 25 }, { fontSize: 17, bold: true, color: C.navy, alignment: "center" });
  ...
  const values = [
    ...
    ["Delta Dental of California / relevant unit", "Toothpic / quip (historical)", "VALIDATE", "VALIDATE"],
