...
OPENING_EN = (
    ...
    "We are already receiving Snowflake startup support [official program name and current sponsor: confirm before release], and this proposal converts that relationship into a structured 90-day joint validation.\n\n"
...
ACTIONS_EN = [
    ...
    ["Run 2.5K / 10K / 25K workload sizing", "Solution Engineer", "Data lead", "Day 30", "Assumptions and post-credit unit economics"],
...
ACTIONS_KO = [
    ...
    ["2.5K / 10K / 25K 규모 산정", "Solution Engineer", "데이터 담당", "30일", "가정과 크레딧 반영 후 단위 비용"],
...
def cover(doc: Document, internal: bool):
    ...
    callout(
        ...
        "기존 Snowflake 지원 관계를 90일 공동 검증으로 전환하는 미국 self-funded employer 치아보험 PoC입니다. [공식 프로그램명·현재 스폰서 확인 필요]" if internal else "This converts ICLO's existing Snowflake startup support into a structured 90-day validation. [Official program name / current sponsor: confirm]",
        PALE_BLUE,
        SKY,
    )
...
def build_english():
    ...
    paragraph(doc, "ICLO helps employees understand and use their dental benefits, navigate to appropriate in-network care, and converts the journey into claims-verified, aggregate evidence for the employer. The current PoC scope is a U.S. self-funded employer dental benefit administered by a dental carrier or TPA.")
    ...
    data_table(doc, ["Field", "Current statement"], [
        ["Company", "[Legal entity, headquarters, stage and core team size: confirm before release]"],
        ...
        ["Snowflake relationship", "Existing startup support [official program name and current sponsor: confirm]"],
    ...
    callout(doc, "RELEASE GATE", "Replace every bracketed field. Program selection does not prove insurer integration, underwriting, adjudication or U.S. outcomes.", PALE_AMBER, AMBER)
    ...
    callout(doc, "OPERATING + CONSUMPTION MODEL", "ICLO runs weekly written status and one decision log. Annual credits are estimated from employers × member-months × ingestion/transform/outcome-refresh frequency during the sizing session.", PALE_BLUE, SKY)
    ...
    paragraph(doc, OPENING_EN)
    paragraph(doc, "45-minute agenda", "45-minute agenda")
    data_table(doc, ["Minutes", "Topic", "Decision / output"], AGENDA_EN, [20, 48, 104], 7.7)
    ...
    data_table(doc, ["Action", "Snowflake owner", "ICLO owner", "Due date", "Required output"], ACTIONS_EN, [48, 32, 30, 28, 42], 6.1)
    ...
    sources = [
        "U.S. Department of Labor - group plans may include dental and may purchase insurance or self-fund: https://www.dol.gov/agencies/ebsa/about-ebsa/our-activities/resource-center/publications/understanding-your-fiduciary-responsibilities-under-a-group-health-plan",
        "U.S. Department of Labor - self-insured sponsor pays claims directly; fully insured sponsor pays premiums and the insurer assumes risk: https://beta.dol.gov/research-data/surveys-reports-publications/2026-report-congress-annual-report-self-insured-group-health-plans",
        ...
        "Medicaid.gov - pediatric/CHIP dental requirements and state-option adult dental: https://www.medicaid.gov/medicaid/benefits/prevention",
...
def build_korean():
    ...
    page_break(doc)
    section_title(doc, "대표·임원 검토 요약", "PoC 범위: 미국 Self-Funded Employer의 치아보험", "B-prime의 문제 제기로 시작하되, A안인 Snowflake 협업·근거 레이어를 본체로 유지했습니다.")
    data_table(doc, ["검토 항목", "이번 문서의 반영"], [
        ["메인 방향", "Snowflake와 함께 만들 근거 레이어가 중심입니다. 미국 시장 문제는 오프닝에서만 사용합니다."],
        ["현재 PoC 범위", "미국 self-funded employer + 치아보험사·TPA + 임직원·가족입니다."],
    ...
    paragraph(doc, "ICLO의 핵심 정의: 임직원이 치아보험 혜택을 이해하고, 네트워크 안의 적절한 치과를 찾고, 필요한 지원을 요청하도록 돕습니다. 그 뒤의 이용 여정을 치아보험 청구 데이터로 확인해 기업에는 집계된 근거만 제공합니다.")
    callout(doc, "임직원 가치", "임직원은 치아보험을 더 쉽게 이해하고 이용할 수 있습니다.", PALE_BLUE, SKY)
    callout(doc, "기업 의사결정", "기업은 플랜·운영사 적합성, 네트워크 접근성, 예방진료 이용, 허용액·보험자 지급·본인부담, 청구 지연·완결성, 이직률과 장기 복지 가치를 데이터로 판단합니다.", PALE_TEAL, TEAL)
    ...
    page_break(doc)
    section_title(doc, "ICLO 기본 정보", "확인된 사실과 외부 공유 전 반드시 채울 항목", "제품·데이터 경계와 PoC 대상은 정했지만 회사·Snowflake 계정 관련 대괄호 항목은 확인이 필요합니다.")
    data_table(doc, ["항목", "현재 문구"], [
        ...
        ["Snowflake 지원 관계", "기존 스타트업 지원 [공식 프로그램명·현재 스폰서 확인]"],
        ["현재 Snowflake 상태", "[계정·클라우드·리전·크레딧·사용량·기술지원 담당자 확인]"],
        ["다음 입증", "미국 self-funded employer 치아보험 데이터 경로 1건과 계정팀 사용사례 검토 1건"],
    ], [48, 124], 7.4)
    callout(doc, "외부 공유 전 확인", "대괄호 항목을 모두 교체합니다. 프로그램 선정은 보험사 연동·인수·심사 또는 미국 성과를 입증하지 않습니다.", PALE_AMBER, AMBER)
    ...
    callout(doc, "운영·사용량 산정", "ICLO가 주간 서면 현황과 단일 의사결정 기록을 관리합니다. 연간 크레딧은 기업 수 × 가입자-월 × 수집·변환·결과 갱신 빈도로 사이징 세션에서 추정합니다.", PALE_BLUE, SKY)
    ...
    paragraph(doc, OPENING_EN)
    data_table(doc, ["시간", "순서", "회의에서 남길 것"], AGENDA_KO, [20, 48, 104], 7.6)

    page_break(doc)
    section_title(doc, "액션 레지스터", "회의가 좋은 대화로만 끝나지 않게 합니다", "각 액션에 Snowflake 담당자, ICLO 담당자, 기한, 필요한 결과물을 남깁니다.")
    data_table(doc, ["Action", "Snowflake owner", "ICLO owner", "Due date", "Required output"], ACTIONS_KO, [48, 32, 30, 28, 42], 6.0)
    ...
    data_table(doc, ["검수 질문", "판정", "반영 위치"], [
        ...
        ["보장 시장·비용 부담·운영 주체를 구분했는가?", "Yes", "미국 치아보험 생태계와 현재 PoC 경로"],
    ...
    sources = [
        "미국 노동부 - 기업 건강 플랜의 치과 혜택과 보험형·자가부담형 선택: https://www.dol.gov/agencies/ebsa/about-ebsa/our-activities/resource-center/publications/understanding-your-fiduciary-responsibilities-under-a-group-health-plan",
        "미국 노동부 - self-insured 기업은 청구를 직접 부담하고 fully insured는 보험사가 위험을 부담: https://beta.dol.gov/research-data/surveys-reports-publications/2026-report-congress-annual-report-self-insured-group-health-plans",
...
if __name__ == "__main__":
    save(build_english(), OUT / "02_EN_External/ICLO-Snowflake-Joint-Validation-Report-v7-EN-External.docx")
    save(build_korean(), OUT / "01_KO_Internal/ICLO-Snowflake-Joint-Validation-Report-v7-KO-Internal.docx")
    print("reports built")
