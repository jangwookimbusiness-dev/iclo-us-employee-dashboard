...
def cover(doc: Document, internal: bool):
    ...
    callout(
        ...
        "임직원은 치아보험을 더 쉽게 이용하고, 기업은 이용·청구 데이터로 플랜·운영사·치과복지를 판단하는 미국 self-funded employer PoC입니다." if internal else "Employees navigate dental benefits more easily; employers use claims evidence to improve plan and workforce-benefit decisions.",
        PALE_BLUE,
        SKY,
    )
...
def build_english():
    ...
    process_row(doc, [
        ...
        ("Days 61-90 | Validate", "One employer or administrator account-team review, partner checklist, next/no-go decision"),
    ...
    data_table(doc, ["Action", "Snowflake owner", "ICLO owner", "Due date", "Required output"], [
        ...
        ["Validate one employer or administrator account-team use case", "Named account owner", "CEO / product lead", "Day 75", "Written architecture and unmet-need feedback"],
...
def build_korean():
    ...
    page_break(doc)
    section_title(doc, "대표·임원 검토 요약", "미국 self-funded employer 치아보험 PoC로 범위를 고정합니다", "B-prime의 문제 제기로 시작하되, A안인 Snowflake 협업·근거 레이어를 본체로 유지했습니다.")
    ...
    page_break(doc)
    section_title(doc, "시장 특성과 문제", "한국과 미국은 치아보험을 운영하는 기본 구조가 다릅니다", "이번 PoC는 미국 전체가 아니라 self-funded employer의 치아보험·임직원 복지 구조를 대상으로 합니다.")
    ...
    process_row(doc, [
        ...
        ("61-90일 | 검증", "기업 또는 운영사 계정팀 한 곳 검토·파트너 체크리스트·진행/보류 결정"),
    ...
    data_table(doc, ["Action", "Snowflake owner", "ICLO owner", "Due date", "Required output"], [
        ...
        ["기업 또는 운영사 계정팀 1곳 검증", "Named account owner", "CEO / 제품 담당", "75일", "구조·미충족 수요 의견"],
...
if __name__ == "__main__":
    save(build_english(), OUT / "02_EN_External/ICLO-Snowflake-Joint-Validation-Report-v5-EN-External.docx")
    save(build_korean(), OUT / "01_KO_Internal/ICLO-Snowflake-Joint-Validation-Report-v5-KO-Internal.docx")
    print("reports built")
