...
const WORK = path.join(ROOT, "tmp/iclo-snowflake-proposal-v5-template");
const OUT = path.join(ROOT, "output/proposal-v5");
...
function sourcesForSlide(slide) {
  ...
  if ([2, 3].includes(slide)) base.push(...MARKET_SOURCES);
  if ([5, 8, 9, 10, 11].includes(slide)) base.push(...SNOW_SOURCES);
...
async function build(lang) {
  ...
  // Slide 4 contains both a journey step numbered "03" and a page number "03".
  // Update only the lowest textbox, which is the page footer.
  const slide4Threes = records.filter((record) => ["textbox", "shape"].includes(record.kind) && record.slide === 4 && record.text === "03");
  if (slide4Threes.length !== 2) throw new Error(`${lang}: expected two \"03\" labels on slide 4`);
  slide4Threes.sort((a, b) => (b.bbox?.y ?? 0) - (a.bbox?.y ?? 0));
  deck.resolve(slide4Threes[0].id).text = "04";
  ...
  const base = isKo ? "ICLO-Snowflake-Joint-Validation-Proposal-v5-KO-Internal" : "ICLO-Snowflake-Joint-Validation-Proposal-v5-EN-External";
