...
> **미결 (2026-08-28) — FHIR Patient Access 가 세 번째 경로인가.** 이 절은 270/271 을
> 유일한 경로로 씁니다. 그런데 CMS Interoperability rule 이 payer 에게 **Patient Access
> API(FHIR)** 를 요구했고 `Coverage`·`ExplanationOfBenefit` 을 노출하는 payer 가 늘고
> 있습니다. 성격이 다릅니다 — 270/271 은 **우리 서버가** trading partner 자격증명으로
> 묻는 것이고, FHIR Patient Access 는 **환자 본인이 인증해서** 자기 데이터를 끌어오는
> 모델(SMART on FHIR authorization)입니다. 그래서 급여 게이트웨이가 아니라 **앱 흐름**에
> 붙고, 자격증명 보관 문제가 아예 없어집니다.
>
> 확인할 것 둘: 순수 dental carrier 가 그 요건 범위에 들어가는지(대개 medical payer
> 대상입니다), 그리고 CMS-0057-F 의 Prior Auth·Provider Access API 시행 일정.
> **이 절이 "270/271 을 못 붙이면 잔여 한도는 표시하지 않는다. 타협 항목이 아니다" 라고
> 못 박았는데, 그 판단은 경로가 둘일 때 내린 것입니다.** 셋째가 있으면 다시 봅니다.
...
**기술 게이트 중** 착수 전에 반드시 닫아야 하는 것은 **13.3절 동의 순서 문제**와 **4.9절 신원 확인** 둘입니다. (법무 게이트는 별개이며 그쪽이 더 앞섭니다. **전체 목록은 16절이 유일한 기준이고 23개 항목입니다** (본문 12 + 16.1 절 11) — 이 괄호는 2026-08-15 까지 그것을 "BAA·리전, HIPAA 역할, 데이터 권리, 기준선 적재" 넷으로 요약하고 있었는데, **`BAA·리전` 은 16절의 번호 항목이 아니라 9절 가정 A5** 이고 요약이 나머지 17개를 가렸습니다. 정본 `start_gates` 가 드는 여섯은 그중 **착수를 막는 부분집합**이며 각 항목에 출처가 달려 있습니다. **2026-08-25 에 넷에서 다섯, 2026-08-28 에 여섯이 되었습니다** (캘리포니아 CMIA — §16-23) — 이 괄호가 §16 항목이 아니라고 바로잡은 그 `BAA·리전` 이 정본에는 아예 없어서 두 문서가 서로 다른 넷을 세고 있었고, `baa_and_region` 을 넣어 닫았습니다. 16절 표 참조.)
